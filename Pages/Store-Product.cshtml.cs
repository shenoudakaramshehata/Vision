...
{
    public class Store_ProductModel : PageModel
    {
        ...
        public List<Product> bestSellerProducts = new List<Product>();
        public Store_ProductModel(IToastNotification _toastNotification, UserManager<ApplicationUser> userManager, IWebHostEnvironment hostEnvironment, IRazorPartialToStringRenderer renderer, CRMDBContext Context, ApplicationDbContext dbContext)
        {
            _context = Context;
        ...
        public async Task<IActionResult> OnGet(long BDId)
        {
            ...
            productCategories = _context.ProductCategories.Where(e => e.ClassifiedBusinessId == ClassifiedBusinessId && e.Isactive == true).ToList();
            products= _context.Products.Include(e => e.ProductCategory).Where(e=>e.ProductCategory.ClassifiedBusinessId== ClassifiedBusinessId && e.IsActive == true).ToList();

            List<OrderItem> orderItems = _context.OrderItems.Include(e=>e.Product).ThenInclude(e=>e.ProductCategory).Where(e => e.Product.ProductCategory.ClassifiedBusinessId == ClassifiedBusinessId).ToList();

            Dictionary<long, long> productSalesCount = new Dictionary<long, long>();

            foreach (OrderItem orderItem in orderItems)
            {
                if (productSalesCount.ContainsKey(orderItem.ProductId))
                {
                    productSalesCount[orderItem.ProductId] += orderItem.ProductQuantity;
                }
                else
                {
                    productSalesCount[orderItem.ProductId] = orderItem.ProductQuantity;
            ...
            int count = 10; // Number of best seller items to retrieve

            List<long> bestSellerProductIds = productSalesCount
            ...
            foreach (var item in bestSellerProductIds)
            {
                var product = _context.Products.Where(e => e.ProductId == item).FirstOrDefault();
                bestSellerProducts.Add(product);
            } 
            //orderItem= _context.OrderItems.Where(e=>e.Product.ProductCategory.ClassifiedBusinessId== ClassifiedBusinessId).OrderByDescending())
        ...
        public IActionResult OnGetProductbycategoryid(int ProductCategoryId)
        {
            var products = _context.Products.Where(c => c.ProductCategoryId == ProductCategoryId && c.IsActive == true).Select(
                  i => new
                  {


                      //ProductName = i.ProductName,
                      ProductName = i.TitleEn,
        ...
        public IActionResult OnGetSortProducts(string selectedValue,int ClassifiedBusinessId)
        {
            if(selectedValue== "price-low")
            {
                var products = _context.Products.Include(e => e.ProductCategory).Where(e => e.ProductCategory.ClassifiedBusinessId == ClassifiedBusinessId && e.IsActive == true).OrderBy(e=>e.Price).Select(
                 i => new
                 {
            ...
            {
                var products = _context.Products.Include(e => e.ProductCategory).Where(e => e.ProductCategory.ClassifiedBusinessId == ClassifiedBusinessId && e.IsActive == true).OrderByDescending(e => e.Price).Select(
                                 i => new
                                 {
