...
{
    public class realestatePostAdTestModel : PageModel
    {
        ...
        public realestatePostAdTestModel(IRazorPartialToStringRenderer renderer, IWebHostEnvironment hostEnvironment, CRMDBContext Context, UserManager<ApplicationUser> userManager)
        {
            _context = Context;
        ...
        public async void OnGet()
        {
            try
            {
                ...
                var ClassifiedCategory = _context.ClassifiedAdsCategories.Where(e => e.ClassifiedAdsCategoryId == 4).FirstOrDefault();

                if (ClassifiedCategory != null)
                {
                    lstColumns = _context.AdTemplateConfigs.Include(c => c.AdTemplateOptions).Include(c => c.FieldType).Where(c => c.ClassifiedAdsCategoryId == 4).OrderBy(c => c.SortOrder).ToList();
                }
                ...
                FormTemplate = System.IO.File.ReadAllText(_hostEnvironment.WebRootPath + "\\AdsTemplates\\4.txt");
        ...
        public async Task<ActionResult> OnPost()
        {
            try
            {
                ...
                var fields = _context.AdTemplateConfigs.Include(c => c.AdTemplateOptions).Include(c => c.FieldType).Where(c => c.ClassifiedAdsCategoryId == 4).OrderBy(c => c.SortOrder).ToList();
